...
library(maps)
#dir.create("D://R_3_0/digiplante/climates/TMIN")
setwd("D://R_3_0/digiplante/climates/TCOMMON/")
years=2000:2016
for (i in years){
  filename=paste("D://R_3_0//digiplante//climates/TCOMMON/TCOMMON_",i,".csv",sep="")
  mapinfo=read.table(filename,sep = ",",header = T)
  mapinfo.x=mapinfo$LONGITUDE;
  mapinfo.y=mapinfo$LATITUDE;
  png(paste("TCOMMON_station_",i,".png",sep=""))
  map("world", fill=TRUE, col="white", bg="lightblue",ylim=c(-60, 90), mar=c(0,0,0,0))
  points(mapinfo.x,mapinfo.y, col="red", pch=16);
  title(paste("TCOMMON_station_",i,sep=""));
  dev.off()
}
